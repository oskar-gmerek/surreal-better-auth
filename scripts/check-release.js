...
function getCurrentVersion() {
  const pkg = JSON.parse(
    readFileSync(join(PACKAGE_PATH, "package.json"), "utf8"),
  );
  return pkg.version;
}
...
function main() {
  ...
  console.log(
    `📁 Git Status: ${git.clean ? "✅ Clean" : "❌ Uncommitted changes"}`,
  );
  if (!git.clean) {
  ...
  if (git.clean) {
    if (branch === "beta") {
      ...
      console.log(
        "→ GitHub Actions will auto-increment beta version and publish",
      );
