...
export class TestHelpers {
  ...
  static async createTeam(page: Page, orgSlug: string, teamName: string) {
    ...
    await page.waitForSelector('button.tab:has-text("Teams")', { timeout: 10000 });
    await page.click('button.tab:has-text("Teams")');
    await page.waitForTimeout(1000);

    // Click Create Team button
    await page.waitForSelector('button:has-text("Create Team")', { timeout: 5000 });
  ...
  static async inviteUserToOrganization(page: Page, orgSlug: string, email: string, role: string = 'member') {
    ...
    await expect(page.locator('h1')).toContainText('Manage Organization', { timeout: 10000 });
    ...
    await expect(page.locator('h1')).toContainText('Members', { timeout: 10000 });
    ...
    await page.waitForSelector('input#invite-email[name="email"]', { timeout: 10000 });
    ...
    try {
      await expect(page.locator('text=Invitation sent')).toBeVisible({ timeout: 15000 });
    } catch (error) {
  ...
  static async acceptInvitationForUser(page: Page, userEmail: string) {
    ...
    if (await acceptButton.isVisible({ timeout: 5000 })) {
      ...
      try {
        await expect(page.locator('text=Invitation accepted')).toBeVisible({ timeout: 3000 });
      } catch {
        await page.waitForTimeout(2000);
  ...
  static async verifyUserInDatabase(email: string, shouldExist: boolean = true) {
  ...
  static async verifyOrganizationInDatabase(slug: string, shouldExist: boolean = true) {
    const db = await this.getDbConnection();
  ...
  static async verifyMembershipInDatabase(userId: string, orgId: string, expectedRole?: string) {
    const db = await this.getDbConnection();
    const result = await db.query<[any[]]>(`SELECT * FROM member WHERE userId = '${userId}' AND organizationId = '${orgId}'`);
  ...
  static async cleanupAllData() {
    ...
    try {
      const tables = ['verification', 'session', 'invitation', 'teamMember', 'member', 'team', 'business', 'account', 'user'];
